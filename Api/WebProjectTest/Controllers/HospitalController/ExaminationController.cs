...
namespace WebProjectTest.Controllers.HospitalController
{
    ...
    public class ExaminationController(IExaminationServices examService, ISqlHelper dal, IRedisCacheService redisCacheService) : AutherController(redisCacheService)
    {
        ...
        public async Task<ApiPageResponse<List<HolExamination>>> GetPageAsync(string? examNo, string? examType, string? patientName, int? isPrinted, DateTime? examDate, int page = 1, int size = 10)
        {
            RefAsync<int> count = 0;
            var res = await examService.GetExaminationPageAsync(examNo, examType, patientName, isPrinted, examDate, page, size, count, OrgId);
