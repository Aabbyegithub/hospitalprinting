...
namespace WebTaskClass.SampleJob
{
    ...
    public class FileAnalysisTask : IJob
    {
        ...
        {
            var _monitorPaths = new List<string>();
            var Pathlist = await _dal.Db.Queryable<HolFolderMonitor>().Where(a=>a.status == 1).Select(a=>a.folder_paths).ToListAsync();
            string[] separatingStrings = ["##"];
            foreach (var item in Pathlist)
            {
                _monitorPaths.AddRange(item.Split(separatingStrings, StringSplitOptions.RemoveEmptyEntries));
            }
            // 验证所有路径是否存在
            ...
            reportMonitor.AddMonitorPaths(_monitorPaths);
